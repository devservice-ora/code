...
public class BeanDataCollection2<E> extends BeanCollectionModel2<E> {
    /** Bean Instance */
    ...
    private Map _mProps = new HashMap();
    ...
    public BeanDataCollection2(RowContext rowCtx, String name, DataFilter filter) {
        ...
        if (rowCtx instanceof DCRowContext) {
            ...
            if (def != null) {
                ...
                if (propertyDescriptor != null) {
                    ...
                    // See also DCBeanDataControl.internalGetDataFilterVO(filter)
        ...
        /** Initialize bean data collection */
        init();
    }
    ...
    protected void init() {
        ...
        /** Initialize the method operation */
        initOperations();
        /** Initialize fetch size */
        final int fetchSize = getDataFilter().getFetchSize();
    ...
    public Object invokeMethodHandlerSize() {
        return (_isAccessor ? BeanUtils.getProperty(_beanInstance, getMethodHandlerSizeName()) :
                invokeMethod(_beanInstance, getMethodHandlerSizeName(), null, null));
    }
        ...
        if (canHandleMethodHandlerSize() && rowCount == 0) {
            ...
            if (result != null) {
                ...
                if (resultCount instanceof Long || resultCount instanceof Integer) {
                    ...
                    _logger.info(getClass().getName() + "." + getMethodHandlerSizeName() + "(" + rowCount +
                                 ")"); // NORES
                } else {
                    _logger.warning("Invalid return type for method " + getMethodHandlerSizeName() +
                                    "Paging will be disabled."); // NORES
                }
            }
            // Set max. rows count
    ...
    public int getFetchStart() {
        int fetchStart = supportsRangePaging() ? getDataFilter().getFetchStart() : getFetchedRowCount();

        // Value(-1) rowset is empty - restart value(0)
    ...
    public int getFetchSize() {
        return (!isLastRow() ? super.getFetchSize() : 0);
    }

    private boolean isLastRow() {
        return (supportsRangePaging() && getRowCount() == (getDataFilter().getFetchStart() + 1));
    }
    ...
    public Collection<E> getCollection() {
        ...
        if (collectionModel == null && getMethodHandlerName() != null) {
            collectionModel = invokeMethodHandler(fetchStart, getFetchSize());
            if (collectionModel != null) {
                _logger.info(getClass().getName() + "." + getMethodHandlerName() + "(" + fetchStart + ", " +
                             collectionModel.size() + ")"); // NORES
