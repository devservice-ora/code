...
public class DefaultFilterableHandler implements FilterableDataControl {
    ...
    public DefaultFilterableHandler(DataControl dc) {
        _dc = dc;
    }
    ...
    public boolean invokeOperation(Map bindingContext, OperationBinding action, DataFilter filter) {
        ...
        if (operation instanceof DCInvokeMethod) {
            ...
            if (iter != null && methodInfo.getDef().getReturnName() != null &&
                ...
                final String providerName =
                    _dc.getName() + "." + iter.getDataControl().getClass().getName() + "." + iter.getName();
                ...
                if (result != null) {
                    DCUtil.putValueInPath((oracle.adf.model.BindingContext)bindingContext,
                                          action.getOperationInfo().getReturnName(), result);
                    return true;
                }
            }
        }
    ...
    public Object invokeAccessor(RowContext rowCtx, String name, DataFilter filter) {
        ...
        if (rowCtx instanceof DCRowContext) {
            ...
            if (def != null) {
                ...
                final String providerName =
                    _dc.getName() + "." + ctx.getRowDataProvider().getClass().getName() + "." + name;
                Object result = _beanDataProvider.getDataProvider(providerName, viewName);
                if (result == null) {
    ...
    //------------------------------------------------------------------------
    // oracle.adf.model.adapter.bean.criteria.CriteriaHandler implementation methods
    //------------------------------------------------------------------------
    ...
    protected BeanDataCollection2 getDataCollection(BeanDataCollection2 dataCollection) {
        ...
        if (canHandle) {
            ...
            // Return null if this operation can't be handled
            dataCollection = null;
        }
    ...
    public List<String> getHandlerMethodNames() {
        return null;
    }
